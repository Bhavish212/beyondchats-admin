...
const SettingsPage = () => {
  ...
  const tabClass = (tab) =>
    `px-4 py-2 text-sm sm:text-base rounded-t-md font-medium transition ${
      tab === activeTab
  ...
  return (
    <motion.div
      ...
      className="max-w-4xl mx-auto mt-6 sm:mt-10 bg-gradient-to-br from-blue-50 to-teal-100 dark:from-gray-900 dark:to-gray-800 rounded-xl shadow-lg"
    >
      {/* Tabs */}
      <div className="flex flex-wrap gap-2 p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-xl">
        {tabs.map((tab) => (
          <div
            key={tab}
      ...
      {/* Tab Content */}
      <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-b-xl space-y-6">
        {activeTab === "Account" && (
        ...
        {activeTab === "Security" && (
          <div className="space-y-5">
            ...
            />
            <button className="w-full sm:w-auto px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition">
              Update Password
            </button>
          </div>
        )}
